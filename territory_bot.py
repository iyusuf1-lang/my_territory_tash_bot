#!/usr/bin/env python3
"""
🗺 Toshkent Territory Bot (V2 - Async & Secure)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ Trek Mini App orqali (GPS auto-tracking)
✅ /api/trek_submit — fetch orqali, xavfsiz (HMAC + Timestamp)
✅ aiosqlite orqali asinxron tezkor ishlash
"""
...
# Diqqat: Tokenni ochiq yozmang! Serverdan yoki .env fayldan oling.
...
def init_db():
    """Bot ishga tushganda bazani sinxron tarzda yaratish"""
...
async def set_team(user_id: int, team: str):
    async with get_db() as db:
        await db.execute("UPDATE users SET team=? WHERE user_id=?", (team, user_id))
...
async def process_referral(new_user_id: int, referrer_id: int):
    if new_user_id == referrer_id:
        return
    async with get_db() as db:
        async with db.execute("SELECT referred_by FROM users WHERE user_id=?", (new_user_id,)) as cursor:
...
def parse_init_data(init_data: str) -> dict | None:
    """Telegram WebApp initData ni xavfsiz tekshirish (auth_date bilan)"""
    if not init_data:
    ...
    try:
        ...
        for pair in init_data.split("&"):
            idx = pair.find("=")
            if idx == -1: continue
            params[unquote(pair[:idx])] = unquote(pair[idx + 1:])

        hash_val = params.pop("hash", None)
        if not hash_val:
            return None

        # 1. Vaqtni tekshirish (Replay attack oldini olish uchun - 24 soat)
        auth_date = int(params.get("auth_date", 0))
        if time.time() - auth_date > 86400:
            logger.warning("🚨 auth_date muddati eskirgan! Kiber hujum xavfi.")
            return None
...
def polygon_area_m2(points: list) -> float:
    ...
    coords = []
    for p in points:
        dx = haversine(lat0, lng0, lat0, p["lng"]) * (-1 if p["lng"] < lng0 else 1)
        dy = haversine(lat0, lng0, p["lat"], lng0) * (-1 if p["lat"] < lat0 else 1)
        coords.append((dx, dy))
...
async def create_zone_circle(user_id, team, lat, lng, radius) -> int:
    ...
    async with get_db() as db:
        ...
        await db.execute("INSERT INTO zone_history (zone_id, to_user, to_team, action) VALUES (?, ?, ?, 'created')",
                         (zone_id, user_id, team))
        await db.execute("UPDATE users SET zones_owned = zones_owned + 1 WHERE user_id=?", (user_id,))
    return zone_id

async def create_zone_circle_with_photo(bot, user_id, team, lat, lng, radius) -> int:
    zone_id = await create_zone_circle(user_id, team, lat, lng, radius)
    photo_url = await get_user_photo_url(bot, user_id)
    if photo_url: await update_zone_photo(zone_id, photo_url)
    return zone_id

async def create_zone_polygon(user_id, team, points) -> int:
...
async def create_zone_polygon_with_photo(bot, user_id, team, points) -> int:
    zone_id = await create_zone_polygon(user_id, team, points)
    photo_url = await get_user_photo_url(bot, user_id)
    if photo_url: await update_zone_photo(zone_id, photo_url)
    return zone_id

async def capture_zone(zone_id, new_owner, new_team) -> dict | None:
    async with get_db() as db:
        async with db.execute("SELECT * FROM zones WHERE id=?", (zone_id,)) as cursor:
            row = await cursor.fetchone()
            if not row: return None
            z = dict(row)
            
        await db.execute("UPDATE zones SET owner_id=?, team=?, photo_url=NULL WHERE id=?", (new_owner, new_team, zone_id))
        await db.execute("""
            INSERT INTO zone_history (zone_id, from_user, from_team, to_user, to_team, action)
            VALUES (?, ?, ?, ?, ?, 'captured')
        """, (zone_id, z["owner_id"], z["team"], new_owner, new_team))
        
        await db.execute("UPDATE users SET zones_owned = MAX(0, zones_owned - 1) WHERE user_id=?", (z["owner_id"],))
        await db.execute("UPDATE users SET zones_owned = zones_owned + 1, zones_taken = zones_taken + 1 WHERE user_id=?", (new_owner,))
    return z

async def get_all_zones() -> list:
    async with get_db() as db:
        async with db.execute("SELECT * FROM zones WHERE active=1") as cursor:
            rows = await cursor.fetchall()
            return [dict(r) for r in rows]

async def get_user_zones(user_id) -> list:
    async with get_db() as db:
        async with db.execute("SELECT * FROM zones WHERE owner_id=? AND active=1", (user_id,)) as cursor:
            rows = await cursor.fetchall()
            return [dict(r) for r in rows]
...
async def update_zone_photo(zone_id: int, photo_url: str):
    async with get_db() as db:
        await db.execute("UPDATE zones SET photo_url=? WHERE id=?", (photo_url, zone_id))

async def get_zone_history(zone_id) -> list:
    async with get_db() as db:
        async with db.execute("SELECT * FROM zone_history WHERE zone_id=? ORDER BY captured_at DESC LIMIT 10", (zone_id,)) as cursor:
            rows = await cursor.fetchall()
            return [dict(r) for r in rows]
...
async def check_and_award(user_id: int, bot, db_user: dict):
    ...
    if db_user["zones_owned"] >= 1: await try_award("first_zone")
    if db_user["zones_owned"] >= 3: await try_award("landlord_3")
    if db_user["zones_owned"] >= 10: await try_award("landlord_10")
    if db_user["zones_taken"] >= 5: await try_award("conqueror_5")
    if db_user["zones_taken"] >= 10: await try_award("conqueror_10")
    if db_user["total_km"] >= 1: await try_award("walker_1km")
    if db_user["total_km"] >= 5: await try_award("walker_5km")
    if db_user["total_km"] >= 10: await try_award("walker_10km")
    if db_user["referral_count"] >= 3: await try_award("referral_3")
...
async def get_user_achievements(user_id: int) -> list:
    async with get_db() as db:
        async with db.execute("SELECT code, earned_at FROM achievements WHERE user_id=? ORDER BY earned_at DESC", (user_id,)) as cursor:
            rows = await cursor.fetchall()
            return [dict(r) for r in rows]
...
async def process_trek(bot, user_id: int, points: list, team: str, closed: bool, dist_m: float) -> str:
    ...
    async with get_db() as db:
        await db.execute("UPDATE treks SET status='cancelled' WHERE user_id=? AND status='active'", (user_id,))
        await db.execute("""
        ...
        await db.execute("UPDATE users SET total_km = total_km + ? WHERE user_id=?", (dist_km, user_id))
...
async def send_onboarding_miniapp(chat_id: int, bot):
    kb_miniapp = InlineKeyboardMarkup([[InlineKeyboardButton("🚀 O'yin haqida ko'proq", web_app={"url": MINI_APP_URL.rstrip("/") + "/onboarding.html"})]])
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.args and len(ctx.args) > 0 and ctx.args[0].startswith("ref_"):
        try:
            ...
            await process_referral(user.id, referrer_id)
        except ValueError:
            pass
...
async def cmd_map(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🌍 *Territory Xaritasi*", parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🌍 Xaritani ochish", web_app={"url": MINI_APP_URL})]])
    )
...
async def cmd_stats(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not db_user: return await update.message.reply_text("❗️ /start bosing.")
    
    team = TEAMS.get(db_user["team"] or "", {"emoji": "❓", "name": "Tanlanmagan"})
    await update.message.reply_text(
        f"📊 *Statistika*\n\n👤 {db_user['first_name']}\n{team['emoji']} {team['name']}\n\n"
        f"🗺 *Zonalar:* {db_user['zones_owned']} ta\n⚔️ *Egallangan:* {db_user['zones_taken']} ta\n"
        f"🏃 *Jami masofa:* {db_user['total_km']:.2f} km\n👥 *Referrallar:* {db_user['referral_count']} ta",
        parse_mode="Markdown"
    )

async def cmd_leaderboard(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    async with get_db() as db:
        async with db.execute("SELECT first_name, team, zones_owned, zones_taken, total_km FROM users ORDER BY zones_owned DESC LIMIT 10") as cursor:
            rows = await cursor.fetchall()
            
    if not rows: return await update.message.reply_text("📋 Hali o'yinchilar yo'q.")
...
async def cmd_zones(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    zones = await get_user_zones(update.effective_user.id)
    if not zones: return await update.message.reply_text("🗺 Hali zona yo'q.", reply_markup=main_menu_kb())
...
async def cmd_history(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        history = await get_zone_history(zone_id)
        if not history: return await update.message.reply_text("Tarix topilmadi.")
        
        text = f"📜 *Zona #{zone_id} tarixi*\n\n"
        for h in history: text += f"• {h['action']} — {h['to_team']} ({h['captured_at'][:16]})\n"
        await update.message.reply_text(text, parse_mode="Markdown")
...
async def cmd_achievements(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    user_achs = await get_user_achievements(update.effective_user.id)
    if not user_achs:
        return await update.message.reply_text("🏅 *Yutuqlar*\n\nHali yutuqlar yo'q. Trek qiling va zona yarating!", parse_mode="Markdown")

    text = "🏅 *Sizning yutuqlaringiz:*\n\n"
    for a in user_achs:
        ach = ACHIEVEMENT_LIST.get(a["code"], {})
        text += f"{ach.get('title', a['code'])}\n  _{ach.get('desc', '')}_\n  📅 {a['earned_at'][:10]}\n\n"
    ...
    if remaining:
        ...
        for r in remaining[:5]: text += f"  {r['title']}\n"
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data.startswith("team:"):
        ...
        await set_team(user_id, team_key)
...
async def api_trek_submit(request: web.Request) -> web.Response:
    ...
    except Exception: return web.Response(text=json.dumps({"ok": False, "error": "Invalid JSON"}), status=400, headers=CORS_HEADERS)

    user_info = parse_init_data(body.get("init_data", ""))
    if not user_info:
        return web.Response(text=json.dumps({"ok": False, "error": "Unauthorized"}), status=401, headers=CORS_HEADERS)

    user_id = user_info.get("id")
    await upsert_user(user_id, user_info.get("username", ""), user_info.get("first_name", ""))

    msg = await process_trek(_app.bot, user_id, body.get("points", []), body.get("team", ""), body.get("closed", False), body.get("distance", 0))

    try: await _app.bot.send_message(chat_id=user_id, text=msg, parse_mode=ParseMode.MARKDOWN, reply_markup=main_menu_kb())
    except Exception as e: logger.error(f"❌ Bot message error: {e}")
...
async def api_zones(request: web.Request) -> web.Response:
    ...
    return web.Response(text=json.dumps(zones, default=str), content_type="application/json", headers=CORS_HEADERS)
...
def main():
    if not BOT_TOKEN:
        logger.error("❌ BOT_TOKEN topilmadi! Atrof-muhit o'zgaruvchilari (.env) dan sozlang.")
        return
